...
is
  ...
  -- For the salt, any literal will do but it must be known whenever the password is re-hashed for 
  -- validation purposes.
  -- A more secure method would be to use a random salt and store it as a separate column along with 
  -- the password.
...
begin
  select 1
    into l_id
    from kst_app_user u
   where u.verification_code = p_verification_code
...
begin
  ...
  update kst_app_user
     set password_hash = l_hashed_password,
...
begin
  select 'Y'
    into l_is_member
    from kst_app_user a
   where upper(a.email) = upper(p_email)
     and a.is_member = 'Y';
  --
  return true;
exception
